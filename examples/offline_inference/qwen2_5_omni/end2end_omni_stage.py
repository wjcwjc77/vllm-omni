...
def get_text_query(question: str = None) -> QueryResult:
    if question is None:
        question = "Please say: Hello, this is a test of text-to-speech generation."
    prompt = (
...
def get_mixed_modalities_query(
    ...
    question = "Please analyze the content and speak your response."
    prompt = (
        f"<|im_start|>system\n{default_system}<|im_end|>\n"
...
def get_use_audio_in_video_query(
    ...
    question = "Please describe the video content and speak your response clearly."
    prompt = (
        f"<|im_start|>system\n{default_system}<|im_end|>\n"
        "<|im_start|>user\n<|vision_bos|><|VIDEO|><|vision_eos|>"
        f"{question}<|im_end|>\n"
...
def get_multi_audios_query(audio_path: Optional[str] = None, sampling_rate: int = 16000) -> QueryResult:
    question = "Please analyze these two audio clips and speak your response."
    prompt = (
        f"<|im_start|>system\n{default_system}<|im_end|>\n"
        "<|im_start|>user\n<|audio_bos|><|AUDIO|><|audio_eos|>"
        "<|audio_bos|><|AUDIO|><|audio_eos|>"
...
def get_image_query(question: str = None, image_path: Optional[str] = None) -> QueryResult:
    if question is None:
        question = "Please describe this image and speak your response."
    prompt = (
        f"<|im_start|>system\n{default_system}<|im_end|>\n"
        "<|im_start|>user\n<|vision_bos|><|IMAGE|><|vision_eos|>"
        f"{question}<|im_end|>\n"
...
def get_video_query(question: str = None, video_path: Optional[str] = None, num_frames: int = 16) -> QueryResult:
    if question is None:
        question = "Please describe this video and speak your response."
    prompt = (
        f"<|im_start|>system\n{default_system}<|im_end|>\n"
        "<|im_start|>user\n<|vision_bos|><|VIDEO|><|vision_eos|>"
        f"{question}<|im_end|>\n"
...
def get_audio_query(question: str = None, audio_path: Optional[str] = None, sampling_rate: int = 16000) -> QueryResult:
    if question is None:
        question = "Please analyze this audio and speak your response."
    prompt = (
        f"<|im_start|>system\n{default_system}<|im_end|>\n"
        "<|im_start|>user\n<|audio_bos|><|AUDIO|><|audio_eos|>"
        f"{question}<|im_end|>\n"
...
def main(args):
    ...
    omni_llm = OmniLLM(
        model=model_name,
        stage_configs_path="vllm_omni/model_executor/stage_configs/qwen2_5_omni.yaml",
    ...
    code2wav_sampling_params = SamplingParams(
        temperature=0.0,  # Deterministic - no randomness for consistent audio
        top_p=1.0,  # Disable nucleus sampling
        top_k=-1,  # Disable top-k sampling
        max_tokens=65536,  # Increased for longer audio generation
        seed=SEED,  # Fixed seed for sampling
        detokenize=True,
        repetition_penalty=1.1,
    )
